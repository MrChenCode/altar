...
// net.Listener.
func (h HTTPDOWN) Serve(s *http.Server, l net.Listener) HttpDownServer {
...
func (h HTTPDOWN) ListenAndServe(s *http.Server) (HttpDownServer, error) {
	addr := s.Addr
	if addr == "" {
		if s.TLSConfig == nil {
			addr = ":http"
...
func ListenAndServe(s *http.Server, hd *HTTPDOWN) error {
	if hd == nil {
		hd = &HTTPDOWN{}
	}
